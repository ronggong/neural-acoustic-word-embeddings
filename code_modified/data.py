...
class Dataset(object):
    ...
    def batch(self, batch_size, max_same=1, max_diff=1, output_shape=2):
        ...
        if isinstance(output_shape, list):
            diff_ids = np.zeros((self.num_examples, max_diff), dtype=np.int32)
            ...
            for ii_example in range(self.num_examples):
                ...
                diff_ids[ii_example, :] = np.random.choice(diff_ids_example, size=max_diff)
            pass
        else:
            # generate a matrix (self.num_examples, max_diff), each element random samples from (0, self.num_classes-1)
            diff_ids = np.random.randint(0, self.num_classes - 1, (self.num_examples, max_diff))
            # generate a (len(self.ids), max_diff) matrix, diff_ids > this matrix element should be added by 1
